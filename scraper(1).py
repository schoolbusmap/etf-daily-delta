...
SOURCES: dict[str, ETFSource] = {
    ...
    "AVUV": ETFSource(
        ...
        browser_url=(
            "https://www.avantisinvestors.com/avantis-investments/"
            "total-holdings/?fund=119&type=etf"
        ),
        ...
        # Do not accept the rendered 50-row preview as a complete fund snapshot;
        # the browser must capture/download the complete All Holdings dataset.
        allow_html_table=False,
        aliases={
            "ticker": ("Ticker", "Symbol", "Trading Symbol"),
...
def _dimensional_dfac_urls_from_json(body: bytes, base_url: str) -> list[str]:
    """Extract URLs associated with DFAC Daily Holdings from Document Center JSON.

    Dimensional's document-grid endpoint returns document metadata, not the actual
    holdings rows.  The useful response can contain many unrelated documents, so
    generic URL scraping is too noisy.  Score strings only from JSON objects that
    mention DFAC / US Core Equity 2 and prefer Daily Holdings + CSV/XLS(X) targets.
    """
    try:
    ...
    def visit(node: Any) -> None:
        if isinstance(node, dict):
            # Match the current record's scalar fields only.  Using json.dumps(node)
            # here would make a top-level response object match because one child is
            # DFAC, then accidentally collect URLs from unrelated sibling funds.
            scalar_text = " ".join(
                str(value) for value in node.values()
                if isinstance(value, (str, int, float, bool))
            ).lower()

            fund_match = "dfac" in scalar_text or "us core equity 2" in scalar_text
            daily_match = "daily holdings" in scalar_text or ("daily" in scalar_text and "holding" in scalar_text)
            if fund_match and daily_match:
                for raw in _iter_string_values(node):
                    ...
                    if "dfac" in ul:
                        score += 45
                    if "us-core-equity-2" in ul or "us_core_equity_2" in ul:
                        score += 40
                    if "daily" in ul:
                        score += 25
                    if "holding" in ul:
...
def browser_candidates(source: ETFSource) -> list[DownloadedDocument]:
    ...
    def add_response_doc(response: Any) -> None:
        ...
        try:
            ...
            if source.ticker == "DFAC" and "json" in ct:
    ...
    with sync_playwright() as p:
        ...
        # The generic "Holdings" control below is intentionally not trusted for DFAC.
        if source.ticker == "DFAC":
            _playwright_click_dfac_daily_holdings(page)
